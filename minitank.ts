...
namespace miniTank {
	...
	// export class ReadonlyPWM {constructor(public readonly RPWM: number, public readonly LPWM: number) {
	// }}
	...
	const stateStr = ["Stop", "Forward", "Back"]
	let state = State.Stop

	function adjust(setL: (pwm: number)=>void, setR: (pwm: number)=>void, s: State) {
		adjustL = setL
		adjustR = setR
		state = s
	...
	control.inBackground(function (){
		...
		while (true) {
			...
			if (currentRPWM < lastExpected.RPWM) {
				currentRPWM += deltaPWM
			}
			if (currentRPWM > lastExpected.RPWM) {
				currentRPWM = lastExpected.RPWM
			}
			if (currentLPWM < lastExpected.LPWM) {
				currentLPWM += deltaPWM
			}
			if (currentLPWM > lastExpected.LPWM) {
				currentLPWM = lastExpected.LPWM
			}

			let RV = standardVoltage * currentRPWM
			let LV = standardVoltage * currentLPWM
			let voltageP1 = pins.analogReadPin(AnalogPin.P1)
			if (voltageP1 == 0) {
				bleLog.logLine(stateStr[state] + " --- time:" + convertToText(control.millis()) + "; v:0")
				continue
			}
